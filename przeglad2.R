...
#figA5
por_long <- melt(por_3[,c(41:45,52)], id="day") 
colnames(por_long)[2]="trend"
colnames(por_long)[3]="search_intensity"
...
por_long <- melt(por_3[,c(8, 10,52,53)], id="day") 

colnames(por_long)[2]="trend"
colnames(por_long)[3]="No.page view/articles/tweets"

sp2 <- ggplot(data=por_long, aes(x=day, y=`No.page view/articles/tweets`)) +
  geom_line(aes(linetype=trend, color = trend), size = 1) +
  geom_point(aes(shape=trend, color=trend))+
  scale_x_date(labels = date_format("%d-%m-%y"))+
  theme_bw()+
...
#fig6
data=por_3[,c(46,29,21,35,20,22,2:7)]
s <- c("official incidence","antiviral mask, g", "washing hands, g","quarantine, g", "protective mask, g",  
